 ...
 * @return Errors Status.
 */
static err_status_e update_log_session(void)
{
    int fd;
 ...
 * @return Errors Status.
 */
static err_status_e create_log_session(void)
{
    char path[50];
    struct stat sta;
 ...
 * @return Errors Status.
 */
static err_status_e create_rootfs(void)
{
    struct stat buf;
    int i = 0;
    ...
    if (update_log_session() == E_OK) {
        if (create_log_session() != E_OK) {
            return E_RROR;
        }
    } else {
        return E_RROR;
