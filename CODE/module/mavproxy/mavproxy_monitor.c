...
static void handle_mavlink_command(mavlink_command_long_t* command, mavlink_message_t* msg)
{
    ...
    case MAV_CMD_REQUEST_MESSAGE:{
        mavlink_system_t mav_sys = mavproxy_get_system();
        mavlink_msg_command_long_encode(mav_sys.sysid, mav_sys.compid, msg, command);
        mavproxy_send_immediate_msg(msg, true);
    } break;
...
static err_t handle_mavlink_msg(mavlink_message_t* msg, mavlink_system_t system)
{
    switch (msg->msgid) {
    ...
    case MAVLINK_MSG_ID_PARAM_SET: {
        if (system.sysid == mavlink_msg_param_set_get_target_system(msg)) {
            ...
            if (param_set.param_type != MAV_PARAM_TYPE_REAL32) {
                LOG_W("Unsupported parameter type:%d\n", param_set.param_type);
                break;
            }

            if (mavlink_param_set(param_set.param_id, param_set.param_value) != E_OK) {
                LOG_W("Patameter set failed! Unknown parameter:%s\n", param_set.param_id);
                break;
            }
        }
    } break;
...
//            if (ftp_process_request(ftp_protocol_t.payload, msg->sysid, msg->compid) == E_OK)
            {
                ...
                if (mavproxy_send_immediate_msg(msg, 0) != E_OK) {
...
err_t mavproxy_monitor_create(void)
{
    ...
    thread_mavlink_rx_handle = os_thread_create("mav_rx",
                                                mavproxy_rx_entry,
                                                NULL,
    ...
    /* start rx thread */
    os_thread_startup(thread_mavlink_rx_handle);
