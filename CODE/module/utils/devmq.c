...
//    }
//
//    err_t rt_err = rt_mq_send(nod->handle->mq, msg, nod->handle->msg_size);
//    return (rt_err == E_OK) ? E_OK : E_RROR;
//}
//
...
//            while (rt_mq_recv(nod->handle->mq, msg_buffer, nod->handle->msg_size, RT_WAITING_NO) == E_OK) {
