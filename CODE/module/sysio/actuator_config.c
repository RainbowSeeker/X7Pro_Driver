...
static err_t actuator_parse_device(const toml_table_t* curtab, int idx)
{
    err_t err = E_OK;
    int i;
    const char* key;
    ...
    if (toml_string_in(curtab, "protocol", &DEVICE_LIST[idx].protocol) == 0) {
        ...
        if (DEVICE_PROTOCOL_IS(idx, pwm)) {
            ...
            DEVICE_LIST[idx].config = malloc(sizeof(act_pwm_drv_config));
            ...
            DEVICE_LIST[idx].config = malloc(sizeof(act_dshot_drv_config));
...
static err_t actuator_parse_devices(const toml_array_t* array)
{
    int i;
    toml_table_t* curtab;
    err_t err = E_OK;
...
static err_t actuator_parse_mapping(const toml_table_t* curtab, int idx)
{
    err_t err = E_OK;
    toml_array_t* arr;
    int i;
...
static err_t actuator_parse_mappings(const toml_array_t* array)
{
    int i;
    toml_table_t* curtab;
    err_t err = E_OK;
