...
err_t mission_reset(void)
{
    ...
    ERROR_TRY(mcn_publish(MCN_HUB(mission_data), &data_bus));

    return E_OK;
}

err_t clear_mission_data(const char* path)
{
    if (unlink(path) != 0) {
...
err_t load_mission_data(const char* path)
{
    FILE* fp;
    uint16_t cnt = 0;
    ...
    if (fp != NULL) {
        ...
        mission_data = (struct mission_item*)malloc(mission_count * sizeof(struct mission_item));
        if (mission_data == NULL) {
            fclose(fp);
            return E_NOMEM;
        }
...
err_t mission_data_collect(void)
{
    ...
    if (mcn_poll(fms_out_nod)) {
        ...
        ERROR_TRY(mcn_copy(MCN_HUB(fms_output), fms_out_nod, &fms_out));
...
err_t mission_data_init(void)
{
    ERROR_TRY(mcn_advertise(MCN_HUB(mission_data), mission_data_echo));
    ...
    ERROR_TRY(load_mission_data(MISSION_FILE));
    ERROR_TRY(mission_reset());
