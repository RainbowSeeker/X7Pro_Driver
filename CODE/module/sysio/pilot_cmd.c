...
{
    ASSERT(stickMapping[0] && stickMapping[1] && stickMapping[2] && stickMapping[3]);
...
err_t pilot_cmd_map_stick(
    uint8_t yaw_chan, uint8_t thro_chan, uint8_t roll_chan, uint8_t pitch_chan)
{
    ASSERT(yaw_chan >= 1 && yaw_chan <= 16);
    ASSERT(thro_chan >= 1 && thro_chan <= 16);
    ASSERT(roll_chan >= 1 && roll_chan <= 16);
    ASSERT(pitch_chan >= 1 && pitch_chan <= 16);
...
err_t pilot_cmd_init(void)
{
    ...
    SELF_CHECK(mcn_advertise(MCN_HUB(pilot_cmd), echo_pilot_cmd));
    SELF_CHECK(mcn_advertise(MCN_HUB(rc_channels), echo_rc_channels));
    SELF_CHECK(mcn_advertise(MCN_HUB(rc_trim_channels), echo_rc_channels));
