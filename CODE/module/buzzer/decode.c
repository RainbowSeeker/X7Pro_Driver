...
//新的频率表
static uint16_t freq_tab_new[12];

//signature|调号(0-11)       :  是指乐曲升多少个半音演奏;
//octachord|升降八度(-2到+2) :  < 0 降几个八度; > 0 升几个八度
static int buzzer_song_decode_new_freq(uint8_t signature, int8_t octachord)
{
    uint8_t i, j;
...
static int buzzer_song_decode(uint16_t tone, uint16_t length, uint16_t* freq, uint16_t* sound_len, uint16_t* nosound_len)
{
    static const uint16_t div0_len = SEMIBREVE_LEN; // 全音符的长度(ms)
