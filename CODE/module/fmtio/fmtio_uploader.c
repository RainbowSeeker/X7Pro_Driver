...
static err_t send_char(uint8_t c)
{
    uint32_t bytes;

    bytes = device_write(fmtio_dev, OS_WAITING_FOREVER, &c, 1);
    
    return (bytes == 1) ? E_OK : E_RROR;
}

static err_t send(uint8_t* buff, uint32_t size)
{
    uint32_t bytes;

    bytes = device_write(fmtio_dev, OS_WAITING_FOREVER, buff, size);

    return (bytes == size) ? E_OK : E_RROR;
}

static err_t get_sync(int32_t timeout)
{
    uint8_t c[2];
    uint32_t bytes;
...
static err_t get_info(int param, uint8_t* val, uint32_t size)
{
    uint32_t bytes;
...
static err_t program_fs(const char* file_name)
{
    size_t count = 0;
    uint8_t* file_buf;
    err_t ret;
 ...
 */
err_t fmtio_upload(const char* path)
{
    ...
    while (systime_now_ms() - time < 10000) {
        if (sync() == E_OK) {
            ...
            if (program_fs(path ? path : IO_FIRMWARE_NAME) == E_OK) {
                reboot();
            } else {
