...
static bool_t __msh_state = TRUE;
#else
static bool_t __msh_state = FALSE;
#endif
bool_t msh_is_used(void)
{
    return __msh_state;
...
{
    printf("All available shell commands:\n");
    {
...
int msh_exec_module(const char *cmd_line, int size)
{
    ...
    if (strstr(pg_name, ".mo") != NULL || strstr(pg_name, ".MO") != NULL)
    {
        /* try to open program */
...
static int _msh_exec_lwp(char *cmd, size_t length)
{
    int argc;
    int cmd0_size = 0;
    char *argv[FINSH_ARG_MAX];
...
void msh_auto_complete_path(char *path)
{
    ...
    if (full_path == NULL) return; /* out of memory */
