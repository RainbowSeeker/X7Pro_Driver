 ...
 * 2017-12-11     Bernard      Use FREE to instead of free in fd_is_open().
...
static int fd_alloc(struct dfs_fdtable *fdt, int startfd)
{
    ...
    if (idx == fdt->maxfd && fdt->maxfd < DFS_FD_MAX)
    {
        ...
        fds = (struct dfs_fd **)realloc(fdt->fds, cnt * sizeof(struct dfs_fd *));
    ...
    {
        fdt->fds[idx] = (struct dfs_fd *)calloc(1, sizeof(struct dfs_fd));
        if (fdt->fds[idx] == NULL)
            idx = fdt->maxfd;
    }
...
struct dfs_fdtable *dfs_fdtable_get(void)
{
    ...
    lwp = (struct rt_lwp *)os_thread_self()->lwp;
    if (lwp)
        fdt = &lwp->fdt;
    else
        fdt = &_fdtab;
...
int list_fd(void)
{
    ...
    for (index = 0; index < (int)fd_table->maxfd; index ++)
    {
        ...
        if (fd && fd->fops)
        {
            ...
            else if (fd->type == FT_DEVICE)   printf("%-7.7s ", "device");
