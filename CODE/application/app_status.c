...
//
// Created by 19114 on 2023/2/26.
//
#include <common.h>
#include "FMS_types.h"
#include "lib/mavlink/v2.0/mavlink_types.h"
...
__WEAK void vehicle_status_change_cb(uint8_t status);
__WEAK void vehicle_state_change_cb(uint8_t mode);
...
static void send_home_position(void)
{
    ...
    if (mcn_copy_from_hub(MCN_HUB(ins_output), &ins_out) != E_OK) {
        return;
    }
...
void App_Status_Main(void *argument)
{
