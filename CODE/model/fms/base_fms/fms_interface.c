...
static int fms_output_echo(void* param)
{
    ...
    printf("mode:%s reset:%d\n", fms_mode[fms_out.mode % ARRAY_LEN(fms_mode)], fms_out.reset);
...
{
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, THROTTLE_DZ), &FMS_PARAM.THROTTLE_DZ));
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, YAW_DZ), &FMS_PARAM.YAW_DZ));
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, ROLL_DZ), &FMS_PARAM.ROLL_DZ));
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, PITCH_DZ), &FMS_PARAM.PITCH_DZ));
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, XY_P), &FMS_PARAM.XY_P));
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, Z_P), &FMS_PARAM.Z_P));
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, VEL_XY_LIM), &FMS_PARAM.VEL_XY_LIM));
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, VEL_Z_LIM), &FMS_PARAM.VEL_Z_LIM));
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, YAW_P), &FMS_PARAM.YAW_P));
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, YAW_RATE_LIM), &FMS_PARAM.YAW_RATE_LIM));
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, ROLL_PITCH_LIM), &FMS_PARAM.ROLL_PITCH_LIM));
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, L1), &FMS_PARAM.L1));
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, CRUISE_SPEED), &FMS_PARAM.CRUISE_SPEED));
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, TAKEOFF_H), &FMS_PARAM.TAKEOFF_H));
    SELF_CHECK(param_link_variable(PARAM_GET(FMS, ACCEPT_R), &FMS_PARAM.ACCEPT_R));
}
...
void fms_interface_init(void)
{
    ...
    ASSERT(Pilot_Cmd_ID >= 0);
    ASSERT(GCS_Cmd_ID >= 0);
    ASSERT(Auto_Cmd_ID >= 0);
    ASSERT(Mission_Data_ID >= 0);
    ASSERT(FMS_Out_ID >= 0);
