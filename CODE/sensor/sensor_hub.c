...
static void dcm_from_euler(const float rpy[3], float dcm[9])
{
    ...
    float cosPhi = cosf(rpy[0]);
    float sinPhi = sinf(rpy[0]);
    float cosThe = cosf(rpy[1]);
    float sinThe = sinf(rpy[1]);
    float cosPsi = cosf(rpy[2]);
    float sinPsi = sinf(rpy[2]);
...
static int echo_sensor_imu(void *param)
{
    ...
    err = mcn_copy_from_hub((mcn_hub_t *) param, &imu_report);
...
static int echo_sensor_mag(void *param)
{
    ...
    err = mcn_copy_from_hub((mcn_hub_t *) param, &mag_report);
...
static int echo_sensor_baro(void *param)
{
    ...
    err = mcn_copy_from_hub((mcn_hub_t *) param, &baro_report);
...
static int echo_sensor_airspeed(void *param)
{
    ...
    err = mcn_copy_from_hub((mcn_hub_t *) param, &airspeed_report);
...
static int echo_sensor_gps(void *param)
{
    ...
    err = mcn_copy_from_hub((mcn_hub_t *) param, &gps_report);
...
static int echo_sensor_optflow(void *param)
{
    ...
    if (mcn_copy_from_hub((mcn_hub_t *) param, &optflow_report) != E_OK)
    {
        return -1;
    }
...
static int echo_sensor_rangefinder(void *param)
{
    ...
    if (mcn_copy_from_hub((mcn_hub_t *) param, &rf_report) != E_OK)
    {
        return -1;
    }
...
static void imu_rotation_init(uint8_t id)
{
    ...
    if (id == 0)
    {
        float val_acc0_scale[] = {
                PARAM_GET_FLOAT(CALIB, ACC0_XXSCALE), PARAM_GET_FLOAT(CALIB, ACC0_XYSCALE),
                PARAM_GET_FLOAT(CALIB, ACC0_XZSCALE), PARAM_GET_FLOAT(CALIB, ACC0_XYSCALE),
                PARAM_GET_FLOAT(CALIB, ACC0_YYSCALE), PARAM_GET_FLOAT(CALIB, ACC0_YZSCALE),
                PARAM_GET_FLOAT(CALIB, ACC0_XZSCALE), PARAM_GET_FLOAT(CALIB, ACC0_YZSCALE),
                PARAM_GET_FLOAT(CALIB, ACC0_ZZSCALE)
        };
    ...
    else if (id == 1)
    {
        float val_acc1_scale[] = {
                PARAM_GET_FLOAT(CALIB, ACC1_XXSCALE), PARAM_GET_FLOAT(CALIB, ACC1_XYSCALE),
                PARAM_GET_FLOAT(CALIB, ACC1_XZSCALE), PARAM_GET_FLOAT(CALIB, ACC1_XYSCALE),
                PARAM_GET_FLOAT(CALIB, ACC1_YYSCALE), PARAM_GET_FLOAT(CALIB, ACC1_YZSCALE),
                PARAM_GET_FLOAT(CALIB, ACC1_XZSCALE), PARAM_GET_FLOAT(CALIB, ACC1_YZSCALE),
                PARAM_GET_FLOAT(CALIB, ACC1_ZZSCALE)
        };
        /* calculate rotation matrix */
...
static void mag_rotation_init(uint8_t id)
{
    ...
    if (id == 0)
    {
        float val_mag0_scale[] = {
                PARAM_GET_FLOAT(CALIB, MAG0_XXSCALE), PARAM_GET_FLOAT(CALIB, MAG0_XYSCALE),
                PARAM_GET_FLOAT(CALIB, MAG0_XZSCALE), PARAM_GET_FLOAT(CALIB, MAG0_XYSCALE),
                PARAM_GET_FLOAT(CALIB, MAG0_YYSCALE), PARAM_GET_FLOAT(CALIB, MAG0_YZSCALE),
                PARAM_GET_FLOAT(CALIB, MAG0_XZSCALE), PARAM_GET_FLOAT(CALIB, MAG0_YZSCALE),
                PARAM_GET_FLOAT(CALIB, MAG0_ZZSCALE)
        };
        /* Calculate rotation matrix */
    ...
    else if (id == 1)
    {
        float val_mag1_scale[] = {
                PARAM_GET_FLOAT(CALIB, MAG1_XXSCALE), PARAM_GET_FLOAT(CALIB, MAG1_XYSCALE),
                PARAM_GET_FLOAT(CALIB, MAG1_XZSCALE), PARAM_GET_FLOAT(CALIB, MAG1_XYSCALE),
                PARAM_GET_FLOAT(CALIB, MAG1_YYSCALE), PARAM_GET_FLOAT(CALIB, MAG1_YZSCALE),
                PARAM_GET_FLOAT(CALIB, MAG1_XZSCALE), PARAM_GET_FLOAT(CALIB, MAG1_YZSCALE),
                PARAM_GET_FLOAT(CALIB, MAG1_ZZSCALE)
        };
        /* Calculate rotation matrix */
...
static void imu_offset_init(uint8_t id)
{
    ...
    {
        float gyr0_offset[] = {
                PARAM_GET_FLOAT(CALIB, GYRO0_XOFF), PARAM_GET_FLOAT(CALIB, GYRO0_YOFF),
                PARAM_GET_FLOAT(CALIB, GYRO0_ZOFF)
        };
        float acc0_offset[] = {
    ...
    {
        float gyr1_offset[] = {
                PARAM_GET_FLOAT(CALIB, GYRO1_XOFF), PARAM_GET_FLOAT(CALIB, GYRO1_YOFF),
                PARAM_GET_FLOAT(CALIB, GYRO1_ZOFF)
        };
        float acc1_offset[] = {
...
err_status_e advertise_sensor_imu(uint8_t id)
{
    ...
    {
        case 0:
            ERROR_TRY(mcn_advertise(MCN_HUB(sensor_imu0_0), echo_sensor_imu));
            ERROR_TRY(mcn_advertise(MCN_HUB(sensor_imu0), echo_sensor_imu));
            break;
        case 1:
...
err_status_e advertise_sensor_mag(uint8_t id)
{
    ...
    {
        case 0:
            ERROR_TRY(mcn_advertise(MCN_HUB(sensor_mag0_0), echo_sensor_mag));
            ERROR_TRY(mcn_advertise(MCN_HUB(sensor_mag0), echo_sensor_mag));
            break;
        case 1:
            ERROR_TRY(mcn_advertise(MCN_HUB(sensor_mag1_0), echo_sensor_mag));
            ERROR_TRY(mcn_advertise(MCN_HUB(sensor_mag1), echo_sensor_mag));
            break;
        default:
            return E_INVAL;
    }
...
err_status_e advertise_sensor_baro(uint8_t id)
{
    ...
    {
        case 0:
            ERROR_TRY(mcn_advertise(MCN_HUB(sensor_baro), echo_sensor_baro));
            break;
        default:
            return E_INVAL;
    }
...
err_status_e advertise_sensor_airspeed(uint8_t id)
{
    ...
    {
        case 0:
            ERROR_TRY(mcn_advertise(MCN_HUB(sensor_airspeed), echo_sensor_airspeed));
            break;
        default:
            return E_INVAL;
    }
...
err_status_e advertise_sensor_gps(uint8_t id)
{
    ...
    {
        case 0:
            ERROR_TRY(mcn_advertise(MCN_HUB(sensor_gps), echo_sensor_gps));
            break;
        default:
            return E_INVAL;
    }
...
err_status_e advertise_sensor_optflow(uint8_t id)
{
    ...
    {
        case 0:
            ERROR_TRY(mcn_advertise(MCN_HUB(sensor_optflow), echo_sensor_optflow));
            break;
        default:
            return E_INVAL;
    }
...
err_status_e advertise_sensor_rangefinder(uint8_t id)
{
    ...
    {
        case 0:
            ERROR_TRY(mcn_advertise(MCN_HUB(sensor_rangefinder), echo_sensor_rangefinder));
            break;
        default:
            return E_INVAL;
    }
