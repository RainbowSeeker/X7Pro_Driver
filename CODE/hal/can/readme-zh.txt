...
一 CAN Driver 注册
	...
	uint32_t ticks : 配置Can driver timer周期。
	#ifdef RT_CAN_USING_HDR
		uint32_t maxhdr;
	#endif
	...
	2、struct rt_can_ops
	{
		err_t (*configure)(struct rt_can_device *can, struct can_configure *cfg);
		err_t (*control)(struct rt_can_device *can, int cmd, void *arg);
...
二、 CAN Driver 的添加：
	...
	2、 err_t (*control)(struct rt_can_device *can, int cmd, void *arg);
	接口的
		#define RT_CAN_CMD_SET_FILTER      0x13
		#define RT_CAN_CMD_SET_BAUD        0x14
		#define RT_CAN_CMD_SET_MODE        0x15
