...
static err_t probe(uint32_t* gps_baudrate)
{
    ...
    for (i = 0; i < sizeof(baudrates) / sizeof(baudrates[0]); i++) {
        ...
        if (set_baudrate(ubx_decoder.ubx_dev, baudrate) == E_OK) {
...
static err_t configure_by_ubx(uint32_t baudrate)
{
    ...
    if (UBX_TX_CFG_PRT_BAUDRATE != baudrate) {
        if (set_baudrate(ubx_decoder.ubx_dev, UBX_TX_CFG_PRT_BAUDRATE) == E_OK) {
...
static err_t gps_control(gps_dev_t gps_dev, int cmd, void* arg)
{
    switch (cmd) {
...
static size_t gps_read(gps_dev_t gps_dev, gps_report_t* report)
{
    if (ubx_decoder.got_posllh && ubx_decoder.got_velned) {
...
err_t gps_m8n_init(const char* serial_device_name, const char* gps_device_name)
{
    ...
    os_thread_t tid = os_thread_create("gps_probe", gps_probe_entry, str_buffer,  OS_MAX_PRIORITY - 2, 4096);
    ASSERT(tid != NULL);

    SELF_CHECK(os_thread_startup(tid));
