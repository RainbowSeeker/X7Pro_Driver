...
static struct completion convert_cplt;
...
static err_t adc_measure(adc_dev_t adc_dev, uint32_t channel, uint32_t* mVolt)
{
...
//
//    if (completion_wait(&convecplt, TICKS_FROM_MS(ADC_CONVERSION_TIMEOUT_MS)) != E_OK) {
//        return ERROR;
//    }
//
...
static err_t adc_enable(adc_dev_t adc_dev, uint8_t enable)
{
    struct stm32_adc* adc = (struct stm32_adc*)adc_dev->parent.user_data;
...
err_t drv_adc_init(void)
{
//    SELF_CHECK(adc_hw_init());
//
//    adc_interrupt_configure(&stm_adc1);
//
//    completion_init(&convert_cplt);

    adc1.ops = &_adc_ops;

    return hal_adc_register(&adc1, "adc1", DEVICE_FLAG_RDONLY, &stm_adc1);
}